...
function checkSpace(currentElement) {
    if (currentElement === " ") {
        return true;
    } else {
        return false;
...
function checkUpSlash(currentElement) {
    if (currentElement === "|") {
        return true;
    } else {
        return false;
...
function changeSectionHeight(sectionId, symbolsThatNotExist) {
    ...
    let unknownDivHeight = (symbolsThatNotExist.length) * 40 + 100;
    let addSectionHeight = 450;
    switch (sectionId) {
        case "unknownDiv1":
            sectionToChange.style.height = `${unknownDivHeight}px`; 
            break;

        case "section1":
            if ((unknownDivHeight + addSectionHeight) >= 500) {
                sectionToChange.style.height = `${unknownDivHeight + addSectionHeight}px`;
                break
            } else {
...
function deleteInputOfSection1() {
    document.getElementById("ukr_morze").value = ""
}

function deleteUnknownListSection1() {
    symbolsThatNotExist = [];
...
function deleteInputOfSection2() {
    document.getElementById("morze_ukr").value = ""
}

function deleteUnknownListSection2() {
    symbolsThatNotExist = [];
...
function ukrMorseTransleteFunction() {
    ...
    if (symbolsThatNotExist.length !== 0) {
        ...
        let mainerSection = document.getElementById("section1")
        section.style.height = "150px";
        mainerSection.style.height = "600px"
...
function morseUkrTransleteFunction() {
    ...
    if (symbolsThatNotExist.length !== 0) {
        ...
        let mainerSection = document.getElementById("section2")
        section.style.height = "150px";
        mainerSection.style.height = "600px"
    }
}
...
function getResponce() {
    ...
    if (currentResponse === "") {
        frase.innerHTML = "Введіть щось"
    } else {
        frase.innerHTML = "Ваш відгук відправлено!"
        responseList.push(currentResponse)
    }
